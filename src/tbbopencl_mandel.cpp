...
public:
    ...
    int batches;
    int batch = 0;
    Emitter(int dim, int niter, double init_a, double init_b, double step, int batches): 
        tbb::filter(tbb::filter::serial), dim(dim), niter(niter), init_a(init_a), init_b(init_b), step(step), batches(batches) {}
    void *operator()(void *in) {
        ...
        while(batch < batches) {
			...
			int i_parm = 0;
			OpenCLCheckError( clSetKernelArg(kernel, i_parm++, sizeof(int), &b) );
            ...
            return new task_t(b, M, dev_M, kernel, queue, NULL);
        }
        return NULL;
    }
};
...
public:
    int dim;
    Worker(int dim): 
        tbb::filter(tbb::filter::parallel), dim(dim) {}
...
int main(int argc, char **argv) {
    ...
    for (int r=0; r<retries; r++) {
        ...
        Emitter emitter(dim, niter, init_a, init_b, step, ceil(dim / batch_size));
        Worker worker(dim);
